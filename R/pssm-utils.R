...
#' Bind two PSSMs with an optional uniform-probability spacer
#'
#' Concatenates two Position-Specific Scoring Matrices (PSSMs) along positions, optionally inserting a spacer of uniform-probability positions between them and optionally trimming low information-content edges from each PSSM beforehand. The spacer positions have 0 bits of information and therefore do not contribute to PWM scores; they simply enforce a fixed distance between the two motifs.
#'
#' @param pssm1,pssm2 PSSMs to bind. Each should be a data frame or matrix with columns named \code{A}, \code{C}, \code{G}, \code{T}. Additional columns (such as \code{pos}) are ignored.
#' @param gap The number of uniform-probability spacer positions to insert between \code{pssm1} and \code{pssm2}. Must be a non-negative integer. Default is 0 (no spacer).
#' @param bits_thresh Either \code{NULL} (no trimming) or a numeric threshold passed to \code{\link{trim_pssm}} to trim low information-content positions from the beginning and end of each PSSM before binding. Default is 0.1.
#'
#' @return A data frame with columns \code{A}, \code{C}, \code{G}, \code{T} and a 0-based \code{pos} column, with rows ordered as (trimmed) \code{pssm1}, spacer, (trimmed) \code{pssm2}.
#'
#' @examples
#' pssm1 <- data.frame(
#'     A = c(0.9, 0.1, 0.1),
...
#'
#' # bind directly
#' bind_pssms(pssm1, pssm2, bits_thresh = NULL)
#'
#' # bind with a 5-bp neutral spacer between the two motifs
#' bind_pssms(pssm1, pssm2, gap = 5, bits_thresh = NULL)
#'
#' @seealso \code{\link{trim_pssm}}, \code{\link{bits_per_pos}}
#' @export
bind_pssms <- function(pssm1, pssm2, gap = 0, bits_thresh = 0.1) {
    nuc <- c("A", "C", "G", "T")

    if (!is.numeric(gap) || length(gap) != 1 || gap < 0 || gap != as.integer(gap)) {
        cli_abort("{.field gap} must be a single non-negative integer")
    }

    if (!is.null(bits_thresh) && (!is.numeric(bits_thresh) || length(bits_thresh) != 1)) {
        cli_abort("{.field bits_thresh} must be {.val NULL} or a single numeric value")
    }

    prep <- function(p, name) {
        if (!all(nuc %in% colnames(p))) {
            cli_abort("{.field {name}} must have columns {.val A}, {.val C}, {.val G}, {.val T}")
        }
        if (!is.null(bits_thresh)) {
            p <- trim_pssm(p, bits_thresh = bits_thresh)
        }
        as.data.frame(p)[, nuc, drop = FALSE]
    }
    ...
    if (gap > 0) {
        spacer <- as.data.frame(
            matrix(0.25,
                nrow = gap, ncol = 4,
                dimnames = list(NULL, nuc)
            )
        )
        out <- rbind(p1, spacer, p2)
    } else {
        out <- rbind(p1, p2)
    }

    out$pos <- seq_len(nrow(out)) - 1L
    out
}
