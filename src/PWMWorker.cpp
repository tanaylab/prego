...
// Keep the BLAS serial INSIDE this worker thread.
//
...
// dgemm below runs once per sequence inside a RcppParallel worker, so without
// this each worker opens its own full-size OpenMP team: n_workers * omp_max OS
// threads, and the run collapses into scheduler churn.
//
...
static void pin_worker_blas_to_one_thread() {
#ifndef _WIN32
