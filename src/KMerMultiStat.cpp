...
KMerMultiStat::KMerMultiStat(
			...
			int norm_size,
			float norm_factor,
			const vector<vector<float> > &interv_stat,
			int range_min, int range_max,
			int gap_marg,
			const set<string> *pat_filter) :
	m_k(k),
...
void KMerMultiStat::init_flat_stat(const vector<vector<float> > &stats)
{
	...
	if(m_is_train->size() != stats[0].size()) {
		cerr << "ERROR: mismatch sizes between train mask and response stat when adding response to PWML regression" << endl;
		return;
	}
...
void KMerMultiStat::build_kmers(int range_min, int range_max)
{
	...
	for(uint locid = 0; locid < m_sequences->size() ; locid++) {
		...
		m_cur_locid = locid;
		if(locid % 20 == 0) {
			cerr << "Aggrag, proc locid =" << locid << "\r";
		}
		string::const_iterator seq = (*m_sequences)[locid].begin()+range_min;
		string::const_iterator seq_end = (*m_sequences)[locid].begin()+range_max - m_k - m_max_gap_size;
		int cur_length = seq_end - seq;
		m_kmer_counter.clear();
		for(string::const_iterator j = seq;
		    j <= seq_end;
		    ++j) {
			...
			for(int g = m_min_gap_size; g <= m_max_gap_size; g++) {
				if(g == 0) {
					...
					fill_pat(j, m_k, 0, 0);
					if(m_should_filter
					&& m_pat_filter->count(m_cur_pat) == 0) {
						continue;
					}
		...
		for(map<const string, int>::iterator kmer = m_kmer_counter.begin();
			...
			pair<int, vector<float> > &p =
					m_kmer_multi_stat[kmer->first][multi];
			p.first++;
...
void KMerMultiStat::fill_pat(string::const_iterator seq, 
						int k1, int k2, int g)
{
	int i = 0;
...
//	cerr << "aft fill pat " << m_cur_pat << endl;
}
