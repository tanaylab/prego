...
class DnaProbVec {
  ...
  public:
    ...
    void direct_incr_weight(int code, float weight = 1) { m_p[code] += weight; }
    void direct_incr_log_weight(int code, float lweight) { log_sum_log(m_logp[code], lweight); }
    ...
    float dot(DnaProbVec &arg) {
        return (m_p[0] * arg.m_p[0] + m_p[1] * arg.m_p[1] + m_p[2] * arg.m_p[2] +
                m_p[3] * arg.m_p[3]);
    }
    ...
    float get_direct_prob(int ci) const { return (m_p[ci]); }
    void set_direct_prob(int ci, float p) {
        m_p[ci] = p;
        m_logp[ci] = log(p);
    }

    float get_prob(char c) const { return (m_p[encode(c)]); }
    float get_log_prob(char c) const { return (m_logp[encode(c)]); }

    float get_avg_log_prob() const { return ((m_logp[0] + m_logp[1] + m_logp[2] + m_logp[3]) / 4); }
...
class DnaPSSM {
  ...
  public:
    bool is_bidirect() const { return (m_bidirect); }

    void set_bidirect(bool dir) { m_bidirect = dir; }
    ...
    int get_min_range() const { return (m_min_range); }
    int get_max_range() const { return (m_max_range); }

    DnaPSSM() : m_min_range(0), m_max_range(1000000), m_bidirect(false) {}

    DnaPSSM(const DnaPSSM &other)
        : m_chars(other.m_chars), m_min_range(other.m_min_range), m_max_range(other.m_max_range),
    ...
    DnaProbVec &operator[](int id) { return (m_chars[id]); }
    const DnaProbVec &operator[](int id) const { return (m_chars[id]); }

    uint length() const { return (m_chars.size()); }
    uint size() const { return (m_chars.size()); }

    void resize(int new_size);

    void push_back(DnaProbVec &pv) { m_chars.push_back(pv); }

    string::const_iterator max_like_match(const std::string &target, float &best_p,
                                          int &best_dir) const;
    ...
    void update_like_vec(const string &str, vector<float> &old_like, vector<float> &deltas,
                         vector<int1> &dirs);
    ...
    void count_weighted(const string &target, vector<float> &wgts, vector<int1> &dirs,
                        float thresh_wgt);
    void count_log_weighted(const string &target, vector<float> &wgts, vector<int1> &dirs,
                            float thresh_wgt);
