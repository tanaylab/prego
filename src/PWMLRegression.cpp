...
void PWMLRegression::add_responses(const vector<vector<float> > &stats)
{
	...
	if(m_train_mask.size() != stats[0].size()) {
		cerr << "mismatch sizes between train mask and response stat when adding response to PWML regression" << endl;
		return;
	}
	m_train_n = 0;
	vector<float>::iterator i_multi = m_interv_stat.begin();
	int prb_i = 0;
	for(vector<int>::const_iterator mask = m_train_mask.begin();
		 mask != m_train_mask.end();
		 mask++) {
	...
	cerr << "response avg " << m_data_avg[0] << " response var " << m_data_var[0] << endl;
	cerr << "ranges " << m_min_range << " " << m_max_range << " bin " << m_spat_bin_size << endl;
}
...
void PWMLRegression::init_seed(const string &init_mot, int isbid)
{
	...
	int pos = 0;
	for(string::const_iterator i = init_mot.begin(); 
	    i != init_mot.end(); 
	    i++) {
		if(*i == '*') {
...
{
	cerr << "init pwm with size "<< pwm.size() << endl;
	...
	for(int i = 0; i < pwm.size(); i++) {
		...
		cerr << "set pos i to " << m_nuc_factors[i]['A'] << " " << m_nuc_factors[i]['C'] << " " << m_nuc_factors[i]['G'] << " " << m_nuc_factors[i]['T'] << endl;
	}
...
void PWMLRegression::optimize()
{
	...
	for(int phase = 0; phase < m_resolutions.size(); phase++) {
		cerr << "Start phase " << phase << " resol " << m_resolutions[phase] << endl;
		...
		do {
			...
			init_energies();
			take_best_step();
			cerr << "STEP prev " << prev_r2 << " " << m_cur_r2 << endl;
...
void PWMLRegression::init_energies()
{
	...
	cerr << "Will init til energ " << " num of probes " << m_sequences.size() <<endl;
...
void PWMLRegression::update_seq_interval(int prb_id, 
		string::const_iterator min_i, string::const_iterator max_i,
		int sign, int pos)
{
	for(string::const_iterator i = min_i; i < max_i; i++) {
		...
		for(vector<vector<float> >::const_iterator p = 
						m_nuc_factors.begin();
		    p != m_nuc_factors.end();
		    p++) {
			if(!(*j) || *j == 'N' || *j == '*') {
		...
		if(m_bidirect) {
			...
			for(vector<vector<float> >::reverse_iterator 
						p = m_nuc_factors.rbegin();
			    p != m_nuc_factors.rend();
			    p++) {
				if(!(*j) || *j == 'N' || *j == '*') {
					prod = 0;
					break;
...
void PWMLRegression::take_best_step()
{
	...
	for(int pos = 0; pos < max_pos; pos++) {
		...
		for(int step_i = 0; step_i < neigh_size; step_i++) {
			...
			for(vector<NeighStep>::iterator delta = 
						m_cur_neigh[step_i].begin();
			    delta != m_cur_neigh[step_i].end();
			    delta++) {
...
//			cerr << "r2 at step " << step_i << " was " << cur_step_r2 << endl;
	...
	for(int spat_bin = 0; spat_bin < max_spat_bin; spat_bin++) {
		...
		if(m_spat_factors[spat_bin] >= 0) {
			...
			//cerr << "bin " << spat_bin << " del spat r2 " << spat_r2 << endl;
			if(spat_r2 > best_spat_r2) {
				best_spat_r2 = spat_r2;
				best_spat_bin = spat_bin;
				best_spat_diff = -m_spat_factor_step;
	...
	cerr << "best spat step was bin " << best_spat_bin << " diff " << best_spat_diff << " r2 " << best_spat_r2 << endl;
	if(best_r2 == m_cur_r2) {
	...
	if(best_r2 > best_spat_r2) {
		...
		for(vector<NeighStep>::iterator delta = 
					m_cur_neigh[best_step].begin();
		    delta != m_cur_neigh[best_step].end();
		    delta++) {
		...
		float tot = pos_probs['A'] + pos_probs['C'] + 
					pos_probs['G'] + pos_probs['T'];
		cerr << "update pos " << best_pos << " tot = " << tot << endl;
		pos_probs['A'] /= tot;
		pos_probs['C'] /= tot;
		pos_probs['G'] /= tot;
		pos_probs['T'] /= tot;
		...
		cerr << "update spat bin " << best_spat_bin << " diff " << best_spat_diff << endl;
...
//	cerr << "n = " << n << " ex " << ex << " ex2 " << ex2 << " xy " << xy << " pred var " << pred_var << " cov = " << cov << " resp  avg " << m_data_avg << " resp _var " << m_data_var << endl;

	if(std::isnan(tot_r2)) {
		cerr << "Nan at at r2 var " << pred_var << " " << ex << " " << ex2 << " " << m_data_avg[0] << endl;
	}
	return(tot_r2);
}
...
float PWMLRegression::compute_cur_r2_spat() 
{
	...
	float ex = 0;
	float ex2 = 0;
	int max_prb_id = m_interv_stat.size();
	...
	for(int prb_id = 0; prb_id < max_prb_id; prb_id++) {
		if(m_train_mask[prb_id]) {
			...
			for(vector<float>::iterator bin = prb_derivs->begin();
		   	 bin != prb_derivs->end();
		    	bin++) {
				v += *bin * *fact;
				fact++;
...
void PWMLRegression::report_cur_lpwm(ostream &out)
{
	int max_pos = m_nuc_factors.size();
	for(int pos = 0; pos < max_pos; pos++) {
	...
	for(int sbin = 0; sbin < m_spat_factors.size(); sbin++) {
		fprintf(stderr, "%.2f ", m_spat_factors[sbin]);
	}
}
void PWMLRegression::output_pssm(ostream &out, ostream &spat, int psid)
{
	cerr << "will out pwm" << endl;
	int max_pos = m_nuc_factors.size();
	for(int pos = 0; pos < max_pos; pos++) {
		out << psid << "\t" << pos;
		out << "\t" << m_nuc_factors[pos]['A']
		<< "\t" << m_nuc_factors[pos]['C']
		<< "\t" << m_nuc_factors[pos]['G']
		<< "\t" << m_nuc_factors[pos]['T'] << endl;
	}
	int max_spat_bin = m_spat_factors.size();
...
void PWMLRegression::fill_predictions(vector<float> &preds)
{
	...
	init_energies();
	cerr << "done init energ" << endl;
	float r2 = compute_cur_r2_spat();
	cerr << "done comp r2 = " << r2 << endl;
	...
	for(int prb_id = 0; prb_id < m_interv_stat.size(); prb_id++) {
		if(m_train_mask[prb_id]) {
			...
			float predict = factors['A'] * deriv['A'] + 
			  factors['C'] * deriv['C'] +
			  factors['G'] * deriv['G'] +
			  factors['T'] * deriv['T'];

//		cerr << "prb " << prb_id << " deriv " << deriv['A'] << " " << deriv['C'] << " " << deriv['G'] << " " << deriv['T'] << " factors " << factors['A'] << " " << factors['C'] << " " << factors['G'] << " " << factors['T'] << endl;
