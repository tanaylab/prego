...
public:
	...
	KMerMultiStat(
				...
				vector<string> *focus_loci,
				vector<int> *is_train,
				int bin_num, int norm, float norm_factor,
				const vector<vector<float> > &interv_stat,
				int range_min, int range_max,
				int gap_marg = 2,
				const set<string> *pat_filter = 0);

	void init(int k, int degen, int min_gap, int max_gap,
			vector<string> *sequences,
			vector<int> *is_train,
			int norm, float norm_factor,
			const vector<vector<float> > &interv_stat, 
			int range_min, int range_max,
			int gap_marg = 2);
	...
	bool have_kmer(const string &pat) {
		return(m_kmer_multi_stat.count(pat));
	}
};
