...
float DnaProbVec::get_entropy() const {
    return (-m_p[0] * m_logp[0] - m_p[1] * m_logp[1] - m_p[2] * m_logp[2] - m_p[3] * m_logp[3]);
}
...
void DnaPSSM::resize(int sz) { m_chars.resize(sz); }
...
void DnaPSSM::init_from_seed(const string &seed, float prior) {
    ...
    for (string::const_iterator i = seed.begin(); i != seed.end(); i++) {
        ...
        switch (*i) {
        case 'A':
            p->set_direct_prob(0, 1 - 3 * prior);
            break;
        case 'C':
            p->set_direct_prob(1, 1 - 3 * prior);
            break;
        case 'G':
            p->set_direct_prob(2, 1 - 3 * prior);
            break;
        case 'T':
            p->set_direct_prob(3, 1 - 3 * prior);
            break;
        }
        p->normalize();
        p++;
...
float DnaPSSM::get_max_ll() const {
    float logp = 0;
    for (vector<DnaProbVec>::const_iterator p = m_chars.begin(); p < m_chars.end(); p++) {
...
void DnaPSSM::calc_like_rc(const string &target, float &logp) const {
    ...
    string::const_iterator i = target.begin();
    logp = 0;
    for (vector<DnaProbVec>::const_reverse_iterator p = m_chars.rbegin(); p != m_chars.rend();
         p++) {
        char c;
        switch (*i) {
        case 'A':
...
string::const_iterator DnaPSSM::max_like_match(const string &target, float &best_logp,
                                               int &best_dir) const {
        ...
        if (m_bidirect) {
            ...
            for (vector<DnaProbVec>::const_reverse_iterator p = m_chars.rbegin();
                ...
                case 'A':
                    ...
                    break;
                case 'T':
                    logp += p->get_log_prob('A');
                    break;
                case 'C':
                    logp += p->get_log_prob('G');
                    break;
                case 'G':
                    logp += p->get_log_prob('C');
                    break;
...
void DnaPSSM::update_like_vec(const string &target, vector<float> &likes, vector<float> &deltas,
                              vector<int1> &dirs) {
    if (target.length() < m_chars.size()) {
        return;
    }
    ...
    for (string::const_iterator i = target.begin() + m_min_range; i < max_i; i++) {
        ...
        *dir = 1;
        if (m_bidirect) {
            float rlogp = 0;
            j = i;
            for (vector<DnaProbVec>::reverse_iterator p = m_chars.rbegin(); p != m_chars.rend();
                 p++) {
                if (!(*j)) {
                    rlogp = -_REAL(MAX);
                    break;
                ...
                case 'A':
                    ...
                    break;
                case 'T':
                    rlogp += p->get_log_prob('A');
                    break;
                case 'C':
                    rlogp += p->get_log_prob('G');
                    break;
                case 'G':
                    rlogp += p->get_log_prob('C');
                    break;
                case '*':
...
void DnaPSSM::integrate_like_seg(const char *min_i, const char *max_i, float &energy) const {
    ...
    for (const char *i = min_i; i < max_i; i++) {
        const char *j = i;
        float logp = 0;
        for (vector<DnaProbVec>::const_iterator p = m_chars.begin(); p < m_chars.end(); p++) {
            if (!(*j)) {
...
void DnaPSSM::count(string::const_iterator seq, float weight, int dir) {
    if (dir == 1) {
        ...
        for (vector<DnaProbVec>::reverse_iterator i = m_chars.rbegin(); i != m_chars.rend(); i++) {
            char c;
            switch (*seq) {
            case 'A':
                c = 'T';
...
void DnaPSSM::count_weighted(const string &target, vector<float> &wgts, vector<int1> &dirs,
                             float thresh_wgt) {
    ...
    for (string::const_iterator i = target.begin() + m_min_range; i < max_i; i++) {
        ...
        string::const_iterator j = i;
        if (*dir == 1) {
            for (vector<DnaProbVec>::iterator p = m_chars.begin(); p < m_chars.end(); p++) {
                if ((*j) && *j != 'N' && *j != '*') {
                    p->incr_weight(*j, *wgt);
                }
                j++;
            ...
            for (vector<DnaProbVec>::reverse_iterator p = m_chars.rbegin(); p != m_chars.rend();
                ...
                switch (*j) {
                case 'A':
                    p->direct_incr_weight(3, *wgt);
                    break;
                case 'T':
                    p->direct_incr_weight(0, *wgt);
                    break;
                case 'C':
                    p->direct_incr_weight(2, *wgt);
                    break;
                case 'G':
                    p->direct_incr_weight(1, *wgt);
                    break;
                default:
                    break;
                }
...
void DnaPSSM::count_log_weighted(const string &target, vector<float> &wgts, vector<int1> &dirs,
                                 float thresh_wgt) {
    // iterate on the correct range
    if (target.length() < m_chars.size()) {
        return;
    ...
    for (string::const_iterator i = target.begin() + m_min_range; i < max_i; i++) {
        ...
        if (*dir == 1) {
            ...
            for (vector<DnaProbVec>::reverse_iterator p = m_chars.rbegin(); p != m_chars.rend();
                ...
                switch (*j) {
                case 'A':
                    p->direct_incr_log_weight(3, *wgt);
                    break;
                case 'T':
                    p->direct_incr_log_weight(0, *wgt);
                    break;
                case 'C':
                    p->direct_incr_log_weight(2, *wgt);
                    break;
                case 'G':
                    p->direct_incr_log_weight(1, *wgt);
                    break;
                default:
                    break;
                }
...
void DnaPSSM::integrate_energy_max(const string &target, float &energy, vector<float> &spat_func,
    ...
    for (string::const_iterator i = target.begin() + m_min_range; i <= max_i; i++) {
        ...
        if (m_bidirect) {
            ...
            for (vector<DnaProbVec>::const_reverse_iterator p = m_chars.rbegin();
                ...
                switch (*j) {
                case 'A':
                    logp_rev += p->get_log_prob('T');
                    break;
                case 'T':
                    logp_rev += p->get_log_prob('A');
                    break;
                case 'C':
                    logp_rev += p->get_log_prob('G');
                    break;
                case 'G':
            ...
            logp_rev += log(spat_func[spat_bin]);
...
void DnaPSSM::integrate_energy(const string &target, float &energy, vector<float> &spat_func,
                               int spat_bin_size) const {
    if (target.length() < m_chars.size()) {
        energy = -_REAL(MAX);
        return;
...
void DnaPSSM::like_thresh_match(const string &target, float thresh, list<int> &poss,
                                list<float> &vals, list<int> &dirs) {
    if (target.length() < m_chars.size()) {
        return;
    }
