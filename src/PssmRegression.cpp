...
PssmRegression::PssmRegression(
	...
	m_sequences(loci),
	m_relevant_vars(relev),
	m_bidirect(bidir),
	m_pos_coefs(7),
	m_epsilon(eps),
	m_no_neg_mode(no_neg_mode),
	m_min_rms_for_star(min_rms_for_star)
{
...
float PssmRegression::go()
{
	...
	for(int vid = 0; vid < max_vid; vid++) {
		...
		float x = coefs[1]*m_chars[max_pos - 1]['A'] +
			  coefs[2]*m_chars[max_pos - 1]['G'] +
			  coefs[3]*m_chars[max_pos - 1]['C'] +
			  coefs[4]*m_chars[max_pos - 1]['T'];
	...
	for(; i < max_i; i++) {
		...
		if(m_bidirect) {
			...
			if(pos == -1) {
				...
				if(rprod!= 0) {
				   switch(cur_nuc) {
					case 'A': coefs[4] += rprod;
						  break;
					case 'G': coefs[3] += rprod;
						  break;
					case 'C': coefs[2] += rprod;
						  break;
					case 'T': coefs[1] += rprod;
						  break;
					default:  break;
				   }
				}
...
//	cerr << "coefs " << vid << " - " << coefs[1] << " " << coefs[2] << " " << coefs[3] << " " << coefs[4] << endl;
}
...
//		cerr << "TEST\t" << pos << "\t" << m_coefs[i][1] << "\t" << m_coefs[i][2] << "\t" << m_coefs[i][3] << "\t" << m_coefs[i][4] << "\t" << m_interv_data[i] << endl;
//
//	}
...
//	m_chars[pos]['T'] = m_pos_coefs[4];
	...
	cerr << "new coefs " << m_chars[pos]['A'] << " " << m_chars[pos]['G'] 
		<< " " << m_chars[pos]['C'] << " " << m_chars[pos]['T']
		<< " const " << m_base_coef << endl;
}

float PssmRegression::test(const ds_bitvec &test_set, 
...
void PssmRegression::update_yvals(const ds_bitvec &relevant_set, 
					vector<float> &chip_data)
{
	...
	for(int vid = 0; vid < max_vid; vid++) {
		if(relevant_set[vid]) {
			coefs[0] = 0;
			update_coefs(coefs, vid, -1);
//			cerr << "Corr\t" << vid << "\t" << chip_data[vid] << "\t" << coefs[0] << endl;
