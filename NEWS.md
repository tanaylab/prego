...
* New: `calc_freq_local_pwm()` scores every motif in a `MotifDB` against a
  per-position base frequency matrix at every start position - the ensemble
  counterpart of `compute_local_pwm()`.
