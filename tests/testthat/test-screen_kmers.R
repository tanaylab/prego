...
test_that("screen_kmers works with 1D", {
    ...
    res <- screen_kmers(seqs, resp, kmer_length = 7, min_cor = 0.08, return_mat = TRUE, seed = 60427)
    ...
    res_single <- screen_kmers(seqs, resp, kmer_length = 7, min_cor = 0.5, return_mat = TRUE, seed = 60427)
    expect_equal(nrow(res_single), 1)

    res_df <- screen_kmers(seqs, resp, kmer_length = 7, min_cor = 0.08, seed = 60427)
...
test_that("screen_kmers works with 1D with gaps", {
    ...
    res <- screen_kmers(seqs, resp, kmer_length = 7, min_cor = 0.08, return_mat = TRUE, max_gap = 3, seed = 60427)
    expect_true(res["GATAAGA", 1] > 0.5)
    expect_equal(sum(res[, 1] > 0.5), 1)
    expect_true(is.matrix(res))
    expect_true(!is.null(rownames(res)))
    ...
    res_df <- screen_kmers(seqs, resp, kmer_length = 7, min_cor = 0.08, max_gap = 3, seed = 60427)
    expect_equal(colnames(res_df)[1:4], c("kmer", "max_r2", "avg_n", "avg_var"))
    expect_equal(unlist(res_df[, 5]), res[, 1], ignore_attr = TRUE)
    expect_true(res_df[res_df$kmer == "GATAAGA", "avg_n"] - 0.5 <= 1e-2)
})
